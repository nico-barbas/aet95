...
#define GLOBAL_SHADER_DATA_INDEX 0
#define MATERIAL_SHADER_DATA_INDEX 1
...
void init_renderer(
  ...
  renderer->global_group_layout = unwrap(make_gpu_shader_group_layout(
      &(GPU_Shader_Group_Layout_Create_Info){
        .binds = ARRAY_LIT(
            ...
            SHADER_UNIFORM(sizeof(Global_Storage_Data)),
            SHADER_STORAGE(INSTANCE_DATA_BUFFER_SIZE)
        )
      },
      allocator
  ));
  ...
  renderer->default_pipeline = unwrap(make_gpu_pipeline(
      &(GPU_Pipeline_Create_Info){
        ...
        .depth_test = true,
        .layout = renderer->default_layout,
  ...
  // open_map_set(renderer->texture_cache, DEFAULT_MATERIAL_HANDLE,
  // white_texture);
...
void begin_render_3d(Renderer *renderer, Raw_Camera *camera) {
  ...
  gpu_render_pass_bind_group(
      ...
      renderer->global_group_data,
      GLOBAL_SHADER_DATA_INDEX
  );
}
...
//     // Vertex *vertices =
//     //     temp_allocator.alloc(temp_allocator, sizeof(Vertex) *
//     vertex_count)
//     //         .allocation;
...
//   // FIXME(nico): for completeness, even if the gpu abstraction asserts on
//   write
...
//   // NOTE(nico): this is a bit dodgy.. I haven't decided how this case should
//   be
//   // handled
...
void init_debug_renderer(
  ...
  assert(renderer->cpu_vertices.items != nullptr);

  renderer->global_group_layout = unwrap(make_gpu_shader_group_layout(
      &(GPU_Shader_Group_Layout_Create_Info){
        .binds = ARRAY_LIT(
            GPU_Shader_Bind_Info, SHADER_UNIFORM(sizeof(Global_Storage_Data)),
        )
      },
      allocator
  ));
  ...
  renderer->layout =
      unwrap(make_gpu_shader_layout(&(GPU_Shader_Layout_Create_Info){
        .groups =
            ARRAY_LIT(GPU_Shader_Group_Layout, renderer->global_group_layout),
      }));

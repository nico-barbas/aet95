...
make_model_from_gltf_file(Model_Create_Info *info, Allocator allocator) {
  ...
  for (usize i = 0; i < data->nodes_count; i += 1) {
    ...
    if (string_equal(name, info->gltf.model_name) && node->mesh != nullptr) {
      mesh = node->mesh;
      break;
    }
  }
  ...
  for (usize i = 0; i < model.primitive_count; i += 1) {
    ...
    model.primitives[i] = (Mesh_Primitive){
      ...
      .gpu_vertices = gpu_buffer_append(
          info->gpu_allocator, vertices.items, sizeof(Vertex) * vertices.len
      ),
      .gpu_indices = gpu_buffer_append(
...
Model_Create_Result make_model(Model_Create_Info *info) {
  switch (info->kind) {
  case Model_Create_Source_Raw_Geometry:
    return make_model_from_raw_geometry(info);
  case Model_Create_Source_GLTF_File:
    assert(false);
  }
...
make_cube_model(GPU_Buffer *gpu_allocator, Material_Handle default_material) {
  ...
  return make_model_from_raw_geometry(&(Model_Create_Info){
    .gpu_allocator = gpu_allocator,
    .kind = Model_Create_Source_Raw_Geometry,
    .raw = {
      .cpu_vertices = (Vertex_Array){.items = vertices, .len = 24},
      .cpu_indices = (Index_Array){.items = indices, .len = 36},
...
make_plane_model(GPU_Buffer *gpu_allocator, Material_Handle default_material) {
  ...
  return make_model_from_raw_geometry(&(Model_Create_Info){
    ...
    .kind = Model_Create_Source_Raw_Geometry,
    .raw = {
      .cpu_vertices = (Vertex_Array){.items = vertices, .len = 4},
      .cpu_indices = (Index_Array){.items = indices, .len = 6},
      .default_material = default_material,
    }
  });
}
